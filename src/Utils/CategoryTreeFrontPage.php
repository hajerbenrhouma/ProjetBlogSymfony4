...
class CategoryTreeFrontPage extends CategoryTreeAbstract
{
    ...
    public function getCategoryListAndParent(int $id): string
    {
        ...
        $key = array_search($id, array_column($this->categoriesArrayFromDb,
            'id'));
    ...
    public function getCategoryList(array $categories_array)
    {
        ...
        foreach ($categories_array as $value) {
            ...
            $url = $this->urlgenerator->generate('videoListe',
                ['categoryname' => $catName,
                    'id' => $value ['id']]);
            $this->categorylist .= $this->html_2 . $this->html_3 . $url . $this->html_4 .
                $catName . $this->html_5;
    ...
    {
        $key = array_search($id, array_column
        ($this->categoriesArrayFromDb, 'id'));
        if ($this->categoriesArrayFromDb[$key]['parent_id'] != null) {
            return $this->getMainParent($this->categoriesArrayFromDb[$key]
            ['parent_id']);
        } else {
            return [
